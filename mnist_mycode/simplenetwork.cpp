...
const char *first_image = "2.png";
const char *second_image = "6.png";
const char *third_image = "8.png";

const char *conv1_bin = "W1_new.bin";
const char *conv1_bias_bin = "b1_new.bin";
const char *ip1_bin = "W2_new.bin";
const char *ip1_bias_bin = "b2_new.bin";
...
{
	sFilename = (std::string("simplenetworkData/new/") + std::string(fname));
}
...
class network_t
{
	...
	cudnnTensorFormat_t tensorFormat;
	cudnnHandle_t cudnnHandle;
	cudnnTensorDescriptor_t srcTensorDesc, dstTensorDesc, biasTensorDesc, srcConvTensorDesc, dstConvTensorDesc;
	cudnnTensorDescriptor_t srcActivTensorDesc, dstActivTensorDesc, srcPoolTensorDesc, dstPoolTensorDesc, srcSoftmaxTensorDesc, dstSoftmaxTensorDesc;
	...
	void createHandles()
	{
		...
		checkCUDNN(cudnnCreateTensorDescriptor(&srcActivTensorDesc));
	...
	void destroyHandles()
	{
		...
		checkCUDNN(cudnnDestroyTensorDescriptor(srcActivTensorDesc));
...
public:
	...
	void convoluteForward(const Layer_t<value_type>& conv,
		int n, int c, int h, int w, cudnnConvolutionFwdAlgo_t algo, void* workSpace, size_t sizeInBytes,
		value_type* srcData, value_type** dstData)
	{
		...
		//int filterStrideA[convDims] = { 2,2 };
		...
		//float* tmpDsc = new float[14*14];
		//memset(tmpDsc, 0, sizeof(float) * 14 * 14);

		//checkCudaErrors(cudaMemcpy(tmpDsc, *dstData, sizeof(float) * 14 * 14, cudaMemcpyDeviceToHost));
		//for (int j = 0; j < h; j++)
		//{
		//	for (int i =0; i <w; i++)
		//	{
	...
	void activationForward(int n, int c, int h, int w, value_type* srcData, value_type** dstData)
	{
		...
		/*setTensorDesc(srcActivTensorDesc, tensorFormat, dataType, n, c, h, w);
		setTensorDesc(dstActivTensorDesc, tensorFormat, dataType, n, c, h, w);*/
	...
	int classify_example(const char* fname, const Layer_t<value_type>& conv1, const Layer_t<value_type>& ip1)
	{
		clock_t start, end;
		...
		const int tensorDims = 4;
		int tensorOuputDimA[tensorDims] = { n, c, h, w };
		const int filterDimA[tensorDims] = { conv1.outputs, conv1.inputs,
			conv1.kernel_dim, conv1.kernel_dim };
		...
		{
			convoluteForward(conv1, 1, 1, IMAGE_H, IMAGE_W, algo, workSpace, sizeInBytes, srcData, &dstData);
			activationForward(1, 1, 14, 14, dstData, &srcData);
			poolForward(1, 1, 7, 7, srcData, &dstData);
			fullyConnectedForward(ip1, 1, 1, 7, 7, dstData, &srcData); //여기서만 input NCHW 대입. 나머지 레이어에선 output NCHW 대입.
		...
		std::cout << "\nSingle computing time : " << (end-start) / double(CLOCKS_PER_SEC)*1000 << std::endl;
...
int main(int argc, char *argv[])
{
	...
	if (checkCmdLineFlag(argc, (const char **)argv, "image"))
	{
		...
		Layer_t<float> conv1(1, 1, 2, conv1_bin, conv1_bias_bin, argv[0]);
		Layer_t<float>   ip1(1, 10, 7, ip1_bin, ip1_bias_bin, argv[0]);
		int i1 = mnist.classify_example(image_name, conv1, ip1);
